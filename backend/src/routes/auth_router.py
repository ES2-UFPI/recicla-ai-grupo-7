...
async def signup(user: user_schema.UserSignUp, session: Session = Depends(get_db)):
    if len(user.password.encode("utf-8")) > MAX_PASSWORD_BYTES:
        return JSONResponse(
            ...
            content=dict(
                return_schema.ReturnError(
                    errors=["Senha muito longa. Máximo permitido é 72 caracteres."]
                )
            ),
        )
...
async def login(user: user_schema.UserLogin, session: Session = Depends(get_db)):
    ...
    if not hash_providers.verify_hash(user.password, str(user_query.password)):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content=dict(return_schema.ReturnError(errors=["Credenciais inválidas."]))
        )
    ...
    user_repo.User(session).save_refresh_token(str(user_query.id), refresh_token)
    user_repo.User(session).activate_user(str(user_query.id))

    return return_schema.ReturnTrueData(
        data=user_schema.Token(
...
async def refresh_token(
    token_data: user_schema.RefreshTokenRequest,
    ...
    # evita usar bool(Column[bool]); comparação explícita é mais segura
    ...
    new_refresh_token = token_providers.create_refresh_token({"sub": str(user.id)})

    user_repo.User(session).save_refresh_token(str(user.id), new_refresh_token)
...
async def get_me(
    session: Session = Depends(get_db),
    user: user_schema.UserOut = Depends(get_logged_user),
):
    return return_schema.ReturnTrueData(data=user)
